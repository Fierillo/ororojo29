...
export const images = {
  ...
  get: async (id: number) => {
    const result = await pool.query<Pick<Image, 'mime_type' | 'data'>>('SELECT mime_type, data FROM images WHERE id = $1', [id]);
    return result.rows[0] || null;
  },
};
...
export const siteConfig = {
  get: async () => {
    const result = await pool.query<{ value: AdminData }>("SELECT value FROM site_config WHERE key = 'admin_data'");
    return (result.rows[0]?.value as AdminData) || {};
  },

  update: async (data: AdminData) => {
    await pool.query(
